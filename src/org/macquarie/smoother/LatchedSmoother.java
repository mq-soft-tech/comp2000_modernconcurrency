...
public class LatchedSmoother {
    ...
    static int[] template = { 1, 2, 4, 8, 16, 32, 64, 16, 0, -32, 0, 16, 64, 32, 16, 8, 4, 2, 1 };
     ...
     * pattern.
     * 
     * This version of parallel smoothing is implemented by explicitly splitting the incoming 
     * image into a number of separate pieces, making and starting thread to evaluate the
     * `smooth()` function on each piece, and using a `CountDownLatch` to waitr until all worker
    ...
    public static void parallelSmoother(int[] in, int start, int end, int[] out) {
        ...
        CountDownLatch stopper = new CountDownLatch(NUM_THREADS);

        int len = end - start;

        int subProblemSize = len / NUM_THREADS;
        if (len % NUM_THREADS != 0)
            subProblemSize++;

        for (int i = 0; i < NUM_THREADS; i++) {
            int subStart = i * subProblemSize;
            int subEnd = Math.min(subStart + subProblemSize, len);
            new Thread(() -> {
                try {
                    starter.await();
                ...
                smoother(source, subStart, subEnd, target);
                stopper.countDown();
            }).start();
        }
